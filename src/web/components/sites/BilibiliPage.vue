...
const ACTIONABLE_COMMANDS = ['bilibili-search', 'bilibili-ranking', 'bilibili-feed', 'bilibili-history', 'bilibili-favorite']

const isListCommand = computed(() => {
  return ACTIONABLE_COMMANDS.includes(activeCommand.value)
})

function handleResultAction(event: { action: string; bvid?: string }) {
  const { action, bvid } = event
  if (!bvid) return
  
  let targetCommandId: string
  
  if (action === 'detail') {
    targetCommandId = 'bilibili-video'
  } else if (action === 'comments') {
    targetCommandId = 'bilibili-comments'
  } else if (action === 'download') {
    targetCommandId = 'bilibili-download'
  } else {
    return
  }
  
  externalParamValues.bvid = bvid
  ...
  message.success(`已填充BV号到「${actionNames[action]}」功能`)
}
